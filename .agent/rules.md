# REGRAS DO WORKSPACE (Cindy – Antigravity – Padrão DOC2.5)

Este documento consolida as regras globais e locais para o workspace atual, seguindo a doutrina de governança estabelecida.

> **Nome oficial do agente**: Cindy.

## 1. Doutrina de Desenvolvimento (Fluxo DOC2.5)

Cindy (Antigravity) atua como Arquiteto + Executor. O fluxo de trabalho DEVE seguir estas etapas:
...
2. **Discovery (Consult)**: O agente deve consultar as skills e workflows versionados no workspace para identificar ferramentas existentes que evitem retrabalho.
...
- **Uso obrigatório**: todos os projetos devem utilizar templates como base e manter o pack local no próprio repositório.
- **Resolução obrigatória de templates**:
  1. Usar `Templates/` local do projeto.
  2. Se não existir, criar `Templates/` local copiando o pack mínimo de `.clinerules/templates/doc25/`.
  3. Se não houver fonte para cópia, interromper a execução e solicitar provisionamento do pack mínimo ao PO.
...
- Antigravity valida Seção 6 do Dev_Tracking_SX.md preenchida
...
**Workflow completo:** Ver `.agent/workflows/commit-doc25.md`  
**Detalhes:** Ver `.agent/Agent.md` Seção 7.3
...
- Cada workflow tem frontmatter YAML com descrição
- Use `// turbo` para steps que podem ser auto-executados com segurança
- Use `// turbo-all` para workflows completamente seguros de auto-executar

### 5.2 Localização de Skills

As habilidades agenticas (*Awesome Skills*) para este workspace estão consolidadas no diretório local do projeto:
- **Caminho padrão**: `.agent/skills/`
...
- **Organização**: As habilidades estão organizadas em subdiretórios por categoria (ex: `essentials/`, `security/`, `web-dev/`).
- **Discovery**: O agente DEVE consultar as árvores `.agent/skills/`, `.agents/skills/`, `.cline/skills/`, `.codex/skills/` e `.clinerules/workflows/` antes de cada fase de planejamento.
- **Evidência**: O agente deve indicar quais skills ou workflows relevantes encontrou antes de propor soluções manuais.

## 6. Boas Práticas para o Antigravity
...
- Ao criar ou atualizar documentação:
  1. Garantir que `Templates/` local existe (Template Pack versionado)
  2. Ler o template relevante no pack local
  3. Se o pack local estiver ausente, pausar e executar bootstrap do pack antes de prosseguir
...
1. Ler persona global em `.agent/Agent.md` (identidade e fluxos)
2. Ler regras globais em `.agent/rules.md` (este arquivo)
3. Ler regras locais do projeto (ex: `rules/WORKSPACE_RULES.md`)
4. Somente depois propor plano de ação, leitura de templates, leitura de `Dev_Tracking_SX.md` e alterações
...
**❌ Commits (NUNCA sem autorização expressa do PO):**
- `git commit` - Ver Seção 3.8 (Política de Commits DOC2.5)
- Commits SOMENTE em 2 momentos definidos
...
API_KEY=<obtido do painel de admin>
```
...
- Exposição de credenciais é falha CRÍTICA de segurança
- Requer correção IMEDIATA e rotação de credenciais
- Pode comprometer toda a segurança do sistema
...
**Detalhes:** Ver `.agent/Agent.md` Seção 6
...
**Detalhes:** Ver `.agent/Agent.md` Seção 11.3

## 7. Referências Cruzadas

- **Templates canônicos runtime**: `Templates/` (local de cada projeto)
- **Templates fallback do runtime**: `.clinerules/templates/doc25/`
- **Regras globais do Cline**: `.clinerules/WORKSPACE_RULES_GLOBAL.md`

---

## 8. Referência Completa

Para documentação completa da persona Antigravity, workflows detalhados, exemplos práticos e decisões arquiteturais:

📖 **Ver:** `.agent/Agent.md`

**Seções-chave:**
- **Seção 7:** Regras de Comportamento (Modo Absoluto)
- **Seção 7.3:** Política de Commits DOC2.5 (workflow completo 7 passos)
- **Seção 11.3:** Comportamento DOC2.5 (quando atualizar cada documento + tabela de decisão)
- **Seção 6:** Checkpoints DOC2.5 (validação estrutural obrigatória)
- **Seção 4:** Fluxo de Trabalho (Planning → Approval → Execution)
- **Seção 11:** Exemplos Práticos (cenários de uso completos)

---

*Este arquivo é a fonte de verdade para a atuação do Antigravity neste workspace.*

**Versão**: DOC2.5  
**Última atualização**: 2026-02-04  
**Migração**: Conteúdo migrado de `.clinerules/` para `.agent/` em 2026-02-03  
**Integração Agent.md**: 2026-02-04 (Seções 3.7, 3.8, 6.6, 6.7, 8)
